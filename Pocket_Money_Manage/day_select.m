...
- (void)viewDidLoad
{
    ...
    //現在起動中のデバイスを取得
    NSString *deviceName = [Devise_Size getNowDisplayDevice];
...
-(void)buttonTapped:(UIButton *)button
{
    //年月日を次画面に渡す
    //input_view *input_v;
    input_view *input_v = [self.storyboard instantiateViewControllerWithIdentifier:@"input_view"];
    input_v.year = [NSString stringWithFormat:@"%d", year];
    input_v.month = [NSString stringWithFormat:@"%d", month];
    day = (int)button.tag;
    input_v.day =  [NSString stringWithFormat:@"%d", day];
    
    ////画面遷移
    input_v.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
    [self presentViewController:input_v animated:YES completion:nil];
